...
def download_and_extract(
    ...
    downloaded = 0
    with session.get(document["download_link"], stream=True, timeout=TIMEOUT) as response:
        response.raise_for_status()
    ...
    if not extracted_path.is_file() or extracted_path.stat().st_size == 0:
        raise ValueError(f"Expected extracted file is missing or empty: {extracted_path}")
        ...
        items.append(
            {
                ...
                "exists": extracted_path.is_file() and extracted_path.stat().st_size > 0,
                "size": None,
            }
        )
